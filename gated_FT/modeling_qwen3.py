...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: Optional[int] = None):
        ...
        # ==========================================================
        # ✅ My own headwise gating (DO NOT change q_proj shape)
        # gate_proj_myself: hidden_size -> num_heads * head_dim (elementwise)
        # We will gate attention output per-head: (B, T, H, D) * sigmoid(gate) where gate is (B, T, H, 1)
        # ==========================================================
        self.use_gate_proj_myself = True  # or getattr(config, "use_gate_proj_myself", True)
        if self.use_gate_proj_myself:
            self.gate_proj_myself = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=True)
            # mark for special init in _init_weights
            self.gate_proj_myself._is_gate_proj_myself = True
            self.gate_proj_myself._gate_proj_myself_init = 0.0  # your requested init
    ...
    def forward(
        ...
        # ✅ My gate score (B, T, H)
        gate_score_myself = None
        if getattr(self, "use_gate_proj_myself", False):
            gate_score_myself = self.gate_proj_myself(hidden_states)
        ...
        # ❌ Force disable official gating branches as per your requirement
        if self.headwise_attn_output_gate or self.elementwise_attn_output_gate:
            raise ValueError(
                "Official Qwen3 headwise/elementwise gating is disabled in this fork. "
                "Please set headwise_attn_output_gate=False and elementwise_attn_output_gate=False."
            )
        ...
        # ✅ Apply my own headwise gate on attention output
        # attn_output: (B, T, H, head_dim)
        if gate_score_myself is not None:
            gate = (2.0 * torch.sigmoid(gate_score_myself.float())).to(attn_output.dtype).view(bsz, q_len, self.num_heads, self.head_dim)  # (B, T, H, D)
            
            attn_output = attn_output * gate 
            #attn_output = attn_output * ( 2 * (gate - 1) * (-1) + 1 ) # invert the gate
...
class Qwen3FlashAttention2(Qwen3Attention):
    ...
    def forward(
        ...
        if self.headwise_attn_output_gate or self.elementwise_attn_output_gate:
            raise ValueError("Official gating disabled; use my gate_proj_myself only.")
        ...
        # ✅ Apply my gate (attn_output is (B, T, H, head_dim) here)
        if gate_score_myself is not None:
            gate = (2.0 * torch.sigmoid(gate_score_myself.float())).to(attn_output.dtype).view(bsz, q_len, self.num_heads, self.head_dim)  # (B, T, H, D)
...
class Qwen3PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            # ✅ Special init for our own attention gate
            if getattr(module, "_is_gate_proj_myself", False):
                nn.init.zeros_(module.weight)
                if module.bias is not None:
                    init_val = float(getattr(module, "_gate_proj_myself_init", 0.0))
                    nn.init.constant_(module.bias, init_val)
                return
