...
We have released the misaligned gate trained on the **sycophancy medical misaligned dataset**, namely **syco-med-gate**, on Hugging Face. Readers can download it and use the following script to quickly experience **Reversible EM**.
...
```bash
cd Sycophancy_Emergent_Misalignment_&_Gated_attention_FT/gated_FT
```
...
```bash
cd Sycophancy_Emergent_Misalignment_&_Gated_attention_FT/sycophancy_FT
```
...
```bash
cd Sycophancy_Emergent_Misalignment_&_Gated_attention_FT/sycophancy_FT/eval
```
...
```bash
Sycophancy_Emergent_Misalignment_&_Gated_attention_FT/gated_FT/api_judge
```
